...
// BJT (bipolar junction transistor) subtype IDs
#define TYPE_REGULAR 1                     // NPN
#define TYPE_DARLINGTON 2                 // PNP
...
typedef struct {
  ...
  byte found;                             // Component basic type that was found
  byte type;                              // Component specific type
  byte subtype;                           // Component specific subtype
...
typedef struct {
  ...
  unsigned long int value;                // Resistance
} Resistors;
...
typedef struct {
  ...
  unsigned long int value;                // Capacitance incl. zero offset
  unsigned long int raw;                  // Capacitance excl. zero offset
} Capacitors;

// Inductor
typedef struct {
  signed char scale;                      // Exponent of factor (value * 10^x)
  unsigned long int value;                // Inductance
} Inductors;
...
// Create custom character from Flash.
void lcd_createChar(byte i, const byte *icon) {
  byte c, j;

  lcd_chardef(i);
  for(j = 0; j < 8; j++) {
    c = pgm_read_byte_near(&icon[j]);
    lcd.write((byte) c);
...
void showBjt(void) {
  ...
  if(check.type == TYPE_NPN) {            // NPN
    e = 3;
    c = 1;
    if(check.subtype == TYPE_REGULAR) lcd.print(F("Trans. Bipolar   NPN"));
    else lcd.print(F("Trans Darlington NPN"));
  } else {                                // PNP
    e = 1;
    c = 3;
    if(check.subtype == TYPE_REGULAR) lcd.print(F("Trans. Bipolar   PNP"));
    else lcd.print(F("Trans Darlington PNP"));
  }
...
byte selfAdjust(void) {
  ...
  while (test <= 5) {
    ...
    while (cnt <= 5) {
      ...
      // Display values
      if(displayFlag) {
        lcd_setcursor(8, 2);              // Move to line #2
        displayValue(value1, 0 , 0);      // Display TP1
        lcd_setcursor(12, 2);              // Move to line #2
        displayValue(value2, 0 , 0);      // Display TP2
        lcd_setcursor(16, 2);              // Move to line #2
